...
#### N-gram転置索引による全文検索システム設計

**設計概要**:
- ステージ単位でのデータ分割（500KB以下/chunk）
- N-gram（2-gram, 3-gram）転置索引による高速検索
- 段階的データ読み込みでクライアント負荷軽減

**データ構造**:
```
static/data/search/
├── index.json           (~80KB: N-gram転置索引 + メタデータ)
├── chunks/
│   ├── chunk_0.json     (~500KB: ステージ群0-89)
│   ├── chunk_1.json     (~500KB: ステージ群90-179)
│   └── ...
```

**チャンク構造**:
- ステージ境界を保持（1チャンク内で異なるステージのデータが混在しない）
- 各チャンクの冒頭にイベントID、ステージ情報等のメタデータを記録
- ステージ単位での完全なストーリーコンテンツを保持

**転置索引構造**:
```javascript
{
  "inverted_index": {
    "アークナ": [{"chunk": 0, "stages": ["OR-1"]}, {"chunk": 2, "stages": ["ST-1"]}],
    "ドクター": [{"chunk": 1, "stages": ["MB-1", "MB-3"]}]
  },
  "stage_chunk_map": {
    "OR-1": 0, "OR-2": 0, "EP-1": 1
  }
}
```
...
**検索UI仕様**:
- AND検索: スペース区切りで複数キーワードのAND条件
- クエリ表示領域: 実際の検索条件を視覚化（例: "アーミヤ AND ケルシー"）
- リアルタイム検索: 300ms debounceでパフォーマンス最適化

**期待効果**:
- 初期読み込み: ~80KB (現在3.3MBから97.6%削減)
- 検索時読み込み: ~1.2MB平均 (必要チャンクのみ)
- 全文検索対応 + 高い日本語検索精度

**実測データ（全58イベント・492ステージ）**:
- **N-gram転置索引サイズ**: 230KB (2-gram, 3-gram、頻度≥2)
- **総チャンク数**: 13個（平均457KB/chunk）
- **総チャンクサイズ**: 17MB (全ストーリーデータ)
- **N-gram統計**: 
  - 生成N-gram総数: 5,806,539個
  - インデックス内ユニークN-gram: 939,881個
  - 圧縮率: 2,452倍
  - 生成時間: 25.4秒

**基本検索との比較**:
- **基本search.json**: 630KB (1000文字制限)
- **N-gramインデックス**: 230KB (全文対応) → **63%削減**
- **追加チャンクデータ**: 17MB (段階的読み込み)
- **実効的なメリット**: 初期読み込み大幅削減 + 全文検索対応

**実装タスク**:
- [x] バックエンド: `src/generators/ngram_search_index.py` - ステージ単位分割 + N-gram索引生成
- [x] コマンドライン統合: パラメータ化可能なN-gram設定
- [x] パフォーマンスチューニング: 複数設定の自動比較機能
- [ ] フロントエンド: `static/js/ngram_search.js` - 段階的検索処理
- [x] UI: 検索クエリ表示領域の実装（AND検索対応）
- [ ] 話者別検索
- [ ] タグベース検索

**N-gram検索パラメータ**:
- N-gramサイズ: `--ngram-sizes 2,3,4` (デフォルト: 2,3)
- 最小頻度: `--ngram-min-freq 2` (出現頻度フィルタ)
- 最大インデックスサイズ: `--ngram-max-index 100000` (bytes)
- 最大チャンクサイズ: `--ngram-max-chunk 500000` (bytes)
- 性能チューニング: `--ngram-tuning` (複数設定の自動比較)

**使用例**:
```bash
# 基本N-gram検索
python3 build.py --limit 5 --use-ngram

# カスタム設定
python3 build.py --use-ngram --ngram-sizes 2,3,4 --ngram-min-freq 3 --ngram-debug

# 性能チューニング実行
python3 build.py --limit 5 --ngram-tuning
```

#### さらに検討すべき設計詳細

**1. 検索結果ランキングアルゴリズムの改善**:
- **TF-IDF (Term Frequency-Inverse Document Frequency)**: N-gram頻度の重み付け
- **位置ベーススコア**: タイトル・冒頭での出現に高スコア
- **文脈関連性**: 周辺キーワードとの関連度
- **話者重要度**: 重要キャラクターの発言に高スコア

**2. 高度な検索機能**:
- **話者検索**: `speaker:アーミヤ` のような構文対応
- **イベント限定検索**: `event:相見歓` で特定イベント内検索
- **時系列検索**: 日付範囲での絞り込み
- **除外検索**: `-keyword` で特定語句を除外
- **フレーズ検索**: `"完全一致フレーズ"` の対応

**3. 検索パフォーマンス最適化**:
- **インクリメンタル検索**: タイプ中のリアルタイム候補表示
- **検索キャッシュ**: 人気クエリの結果キャッシュ
- **部分文字列索引**: 前方一致・後方一致の高速化
- **検索統計**: 人気キーワード・トレンド分析
...
- **関連検索**: 類似・関連キーワードの提案
- **検索結果ハイライト**: 該当箇所の文脈表示
- **検索結果プレビュー**: ストーリー内容の抜粋表示

**5. 国際化・多言語対応**:
- **同義語辞書**: キャラクター名のバリエーション対応
- **ひらがな・カタカナ・漢字変換**: 表記ゆれ対応
- **英語混在検索**: 英語キャラ名での検索対応

**6. 検索分析・モニタリング**:
- **検索失敗ログ**: 結果0件のクエリ分析
- **検索パフォーマンス監視**: 応答時間・負荷測定
- **検索品質評価**: 関連性スコアの妥当性検証
