...
- 表示方式を制御するためのメニュー機能
  ...
  - ナイトモードの ON/OFF切替え
  - ブックマーク機能
    - ブックマークはブラウザのローカルストレージに保存する
    - 各ストーリーのページで、台詞のブロックをクリックするとブックマークできるようにする
    - メニューからブックマーク一覧を表示できるようにする
...
- [x] フォント切替機能
  ...
  - [x] レスポンシブ対応（モバイル表示調整）
- [x] UI/UXの改善
  - [x] ヘッダーレイアウト最適化（タイトルとメニューの縦配置）
  - [x] ナビゲーションメニューの折り返し対策
  - [x] ブックマーク機能のUX向上（視覚的インジケーター、クリック操作）
...
- [x] ブックマーク機能
  ...
  - [x] ブックマーク一覧ページ生成機能
  - [x] ブックマーク削除・管理機能
  - [x] クリック式ブックマーク操作（台詞ブロック全体をクリック）
  - [x] ブックマークインジケーター表示（右上角の栞マーク）
  - [x] ブックマークページでの検索・フィルタ・エクスポート・インポート機能
  - [x] データ互換性保証（localStorage移行対応）
- [x] プロジェクト設定とドキュメント更新
...
### 概要
ユーザーがストーリーの特定の台詞やシーンをブックマークして、後で簡単に見返せる機能。

### 機能要件
1. **ブックマーク保存**
   - 各台詞ブロックをクリックしてブックマーク追加
   - ローカルストレージを使用してブラウザに保存
   - ブックマーク情報: イベント名、ストーリー名、台詞内容、位置情報

2. **ブックマーク表示**
   - ナビゲーションメニューに「ブックマーク」項目追加
   - ブックマーク一覧ページの生成
   - 各ブックマークから元のストーリーページへのリンク

3. **ブックマーク管理**
   - ブックマーク削除機能
   - ブックマークの並び替え（追加日時順）
   - ブックマーク数の制限（パフォーマンス考慮）
...
bookmarks = {
  "bookmark_id_1": {
    ...
    position: "dialog-3" // HTML要素のID
  }
}
```

#### 実装ファイル
1. **`static/js/bookmark.js`** - ブックマーク管理クラス
2. **`templates/bookmarks.html`** - ブックマーク一覧ページテンプレート
3. **`src/generators/bookmark_generator.py`** - ブックマークページ生成器
4. **`templates/base.html`** 修正 - ナビゲーションにブックマークメニュー追加
5. **`templates/story.html`** 修正 - 台詞ブロックにブックマーク機能追加

#### UI/UX設計
1. **ブックマークボタン**
   - 台詞ブロックにホバーで表示される⭐アイコン
   - クリックでブックマーク追加/削除のトグル動作
...
#### 実装ステップ
1. ブックマーク管理JavaScriptクラスの作成
2. ストーリーページでの台詞ブロック選択UI実装
3. ナビゲーションメニューへのブックマークリンク追加
4. ブックマーク一覧ページテンプレートの作成
5. ブックマークページ生成器の実装
6. CSS スタイリング（ダークモード対応含む）
